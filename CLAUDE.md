...
Both AI flows converge on the same shape of "end of interview" payload: a report block (`---RAPOR---...---RAPORSON---`), a standardized-CV block (`---STANDARTCV---...---STANDARTCVSON---`), a numeric score, and a hire recommendation, parsed by `finalize_interview()` (text flow) or the `/api/realtime/report` handler (voice flow).

"Could not be evaluated" (`"Değerlendirilemedi"`) means **insufficient data only** — decided upstream (`assess_data_sufficiency()` for voice, the `should_finish` gates for text) before any score is produced. A genuine low score **with sufficient data** is a real reject: `finalize_interview()` maps score `<20` to `"Reddet"` and keeps the AI report. After generation the score is validated server-side: `recompute_and_fix_score()` clamps any per-criterion point over its cap and re-normalizes `score = awarded / evaluated-criteria-cap * 100`, rewriting the report's `TOPLAM PUAN` line to match `interviews.score`. (An earlier rule that coerced *any* `<20` to `"Değerlendirilemedi"` was removed.)
