...
The "TAM FORMAT" report body (the section list from `**Aday:**` through `---STANDARTCVSON---`) is a single source, `REPORT_BODY_SECTIONS` (a `(key, levels, literal_text)` list) rendered by `build_report_body(level, ctx)`. Both `get_system_prompt()` (L1/L3) and `/api/realtime/report`'s `report_prompt` (L2) read from it. **Caveat for future work:** the L2 call site passes a hardcoded `2`, not `candidate_level` — per the Level-3-reuses-L2's-report-path decision, a Level 3 candidate on that endpoint deliberately gets L2's body unchanged. Adding a Level-3-only section (`levels={3}`) to `REPORT_BODY_SECTIONS` alone does **not** make L3 pick it up — the hardcoded `2` must be changed to `candidate_level` first (and the swap re-verified end-to-end), before any `levels={3}` entries are added.
...
### Cost/usage tracking
Every AI call (Claude and OpenAI) should be logged through `record_ai_usage()` (writes `ai_usage_logs` + increments `interviews.total_*_tokens`) so the admin panel can show per-candidate token/cost breakdowns. `AI_PRICING_PER_1M` holds hand-maintained approximate per-model rates for the cost estimate shown in the panel — not a real invoice source; update it when models/pricing change.
